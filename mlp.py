...
def mlp_objective(model, data, labels, loss_function):
    ...
    for i in reversed(range(num_layers - 1)):
        ...
        #######################################################################
        layer_delta[i] = np.matmul(np.transpose(weights[i+1]), np.multiply(layer_delta[i+1], activation_derivatives[i+1]))
    ...
    for i in range(num_layers):
        ...
        #######################################################################
        layer_gradients[i] = np.matmul(np.multiply(layer_delta[i], activation_derivatives[i]), np.transpose(activations[i]))
