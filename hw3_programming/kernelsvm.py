...
def polynomial_kernel(row_data, col_data, order):
    ...
    #############################################
    return np.power(row_data.T.dot(col_data) + 1, order)
...
def rbf_kernel(row_data, col_data, sigma):
    ...
    #############################################
    x = -2 * row_data.T.dot(col_data)
    rTr = np.matmul(np.diagonal(row_data.T.dot(row_data))., np.ones((col_data.shape[1], 1)).T)
    cTc = np.matmul(np.ones((row_data.shape[1], 1)), np.diagonal(col_data.T.dot(col_data)).T)
    sq_dist = rTr + cTc - 2 * x
    return np.exp((-1*sq_dist)/(2*pow(sigma, 2)))
